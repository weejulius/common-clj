...
(defn init-components
  [configs]
  (alter-state-root component/init "init"
                    (fn [config name]
                      (let [sym (-> config name :component)
                            instance ((c/load-sym sym) nil)]
                        instance))
                    configs))
...
(defn start-components
  [configs]
  (alter-state-root component/start "start"
                    (fn [config name]
...
(defn stop-components
  [configs]
  (alter-state-root component/stop "stop"
                    (fn [config name]
                      (let [component (get state name)]
                        component))
                    configs))
...
(defn halt-components
  [configs]
  (alter-state-root component/halt "halt"
                    (fn [config name]
                      (let [component (get state name)]
                        component))
                    configs))
